...
public class H2CodeGeneratorTest {
    ...
    private StrategyConfig.Builder strategyConfig() {
        // 设置需要生成的表名
        return new StrategyConfig.Builder().addInclude("sys_admin", "sys_permission").addTablePrefix("sys_");
    }
    ...
    public void testCustomTemplateName() {
        AutoGenerator generator = new AutoGenerator(DATA_SOURCE_CONFIG);
        generator.strategy(strategyConfig().entityBuilder().formatFileName("%sEntity").mapperBuilder()
            .formatMapperFileName("%sDao").formatXmlFileName("%sXml").controllerBuilder().formatFileName("%sAction")
