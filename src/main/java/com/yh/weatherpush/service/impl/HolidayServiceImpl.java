...
public class HolidayServiceImpl extends ServiceImpl<HolidayMapper, Holiday> implements HolidayService {
    ...
    @Override
    public List<Holiday> getHolidayFromGitHub(LocalDate date) {
        String year = String.valueOf(date.getYear());
        String url = holidayUrl.replace("{year}", year);
                ...
                if (days != null) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        Holiday holiday = Holiday.builder().holidayName(name).ctime(LocalDateTime.now())
                            .holidayDate(LocalDate.parse(dateStr, format)).isOffDay(isOffDay).year(year).build();
                        list.add(holiday);
                    }
                    saveBatch(list);
                    return list;
